...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const subject = await createSampleSubject();
...
async function createSampleClasses(): Promise<Class[]> {
  return Promise.all(
    ["1DA", "1DB", "1DC", "1DD"].map((name) =>
      prisma.class.upsert({
        where: { name },
        update: {},
        create: { name, year: 1 },
...
async function createSampleSubject(): Promise<Subject> {
  return prisma.subject.upsert({
    where: { code: "TEST_MATCHES" },
    update: {},
    create: {
      code: "TEST_MATCHES",
      name: "Test Matches",
